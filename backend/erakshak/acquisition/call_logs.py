"""Acquire call logs from the Android device.

Attempts to query call logs directly via the Android content provider using ADB.
If blocked by Android's security model (SecurityException, permission denied,
etc.), it falls back to importing exported call logs from the companion
collector app's output.

Output artefacts
----------------
- ``raw/system/content_call_log.txt``    – verbatim content query output (if successful)
- ``raw/collector/calls.jsonl``          – copied collector output (if fallback is used)
- ``derived/call_logs.jsonl``            – normalised call log records (from either source)
"""
...
def acquire_call_logs(
    ...
    First tries querying content://call_log/calls using ADB.
    ...
    query_cmd = ["content", "query", "--uri", "content://call_log/calls"]
    adb_res = adb.shell(
        query_cmd,
        timeout=CONTENT_QUERY_TIMEOUT,
        audit_action="content_query_call_log",
    )
    ...
    if is_adb_successful:
        # Write raw output
        raw_path = case_folder.raw_system_dir / "content_call_log.txt"
        raw_path.write_text(adb_res.stdout, encoding="utf-8")
        manifest.add_file(
            artifact_class="call_logs_raw",
            source_type="adb_command",
            source_command_or_path="adb shell content query --uri content://call_log/calls",
        ...
        # Parse calls
        parsed_calls = parse_content_query(adb_res.stdout)
        results["call_count"] = len(parsed_calls)

        # Write derived calls.jsonl
        derived_path = case_folder.derived_dir / "call_logs.jsonl"
        with open(derived_path, "w", encoding="utf-8") as fh:
            for call in parsed_calls:
                fh.write(json.dumps(call, ensure_ascii=False) + "\n")

        manifest.add_file(
            artifact_class="call_logs",
            source_type="adb_command",
            source_command_or_path="adb shell content query --uri content://call_log/calls",
        ...
        results["source"] = "adb"
        audit.log(
            action="call_logs_acquired_via_adb",
            command_category="call_logs",
    ...
    results["warnings"].append(
        "ADB call logs query blocked or failed. Attempting fallback to collector export."
    )
    audit.log(
        action="call_logs_adb_failed",
        command_category="call_logs",
        result="failed",
    ...
    if collector_folder:
        ...
        src_file = src / "calls.jsonl"
        if src_file.exists():
            is_valid, line_count, err_msg = _validate_jsonl(src_file)
            if is_valid:
                # Copy to raw/collector/
                dest_raw = case_folder.raw_collector_dir / "calls.jsonl"
                shutil.copy2(src_file, dest_raw)
                manifest.add_file(
                    artifact_class="collector_calls",
                ...
                # Copy to derived/call_logs.jsonl
                derived_path = case_folder.derived_dir / "call_logs.jsonl"
                ...
                results["call_count"] = line_count
                results["source"] = "collector"
                audit.log(
                    action="call_logs_acquired_via_collector",
                    command_category="call_logs",
                ...
                results["warnings"].append(
                    f"Collector calls.jsonl exists but has invalid format: {err_msg}"
                )
        else:
            results["warnings"].append("Collector calls.jsonl file not found in export folder.")
    else:
        results["warnings"].append(
            "No collector export folder provided. To acquire call logs on secure devices, "
    ...
    audit.log(
        action="call_logs_acquisition_failed",
        command_category="call_logs",
